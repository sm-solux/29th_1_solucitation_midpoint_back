...
public class EmailController {
    ...
    public ResponseEntity<?> sendVerificationEmailV2(@RequestBody VerificationEmailRequestDto verificationEmailRequestDto) {
        ...
        if (memberService.isEmailAlreadyInUse(verificationEmailRequestDto.getEmail())) {
            return ResponseEntity.badRequest().body(Collections.singletonMap("message", "이미 사용중인 이메일입니다."));
        }
        ...
        return ResponseEntity.ok(Collections.singletonMap("message", "[회원가입] 인증 코드가 이메일로 발송되었습니다. 회원가입 창으로 돌아가 4분 이내에 입력해주세요."));
    }
    ...
    public ResponseEntity<?> verifyCodeV2(@RequestBody VerificationCodeRequestDto verificationCodeRequestDto) {
        ...
        if (isValid) {
            return ResponseEntity.ok(Collections.singletonMap("message", "올바른 인증번호입니다."));
        } else {
            return ResponseEntity.badRequest().body(Collections.singletonMap("message", "유효하지 않거나 만료된 인증번호입니다."));
        }
    }
    ...
    public ResponseEntity<?> sendPwResetEmailV3(@RequestBody PwResetRequestDto pwResetRequestDto) {
        ...
        if (!memberService.isNameAndEmailMatching(pwResetRequestDto.getName(), pwResetRequestDto.getEmail())) {
            return ResponseEntity.badRequest().body(Collections.singletonMap("message", "이름 또는 이메일 정보가 일치하지 않습니다."));
        }
        EmailMessage emailMessage = EmailMessage.builder()
        ...
        return ResponseEntity.ok(Collections.singletonMap("message", "[비밀번호 찾기] 인증 코드가 이메일로 발송되었습니다. 비밀번호 찾기 창으로 돌아가 4분 이내에 입력해주세요."));
    }
    ...
    public ResponseEntity<?> verifyCodeV3(@RequestBody VerificationCodeRequestDto verificationCodeRequestDto) {
        ...
        if (isValid) {
            ...
            // Bearer 접두사 추가하여 리턴
            return ResponseEntity.ok(Collections.singletonMap("token", "Bearer " + accessToken));
        } else {
            return ResponseEntity.badRequest().body(Collections.singletonMap("message", "유효하지 않거나 만료된 인증번호입니다."));
        }
    }
